...
public class MusicRunnable implements Runnable {
	...
	private int min_Comment = 10000;
	// private int get_info_once = Integer.parseInt(new
	// ConfigReader().Read("get_info_once"));;
	// private int min_Comment = Integer.parseInt(new
	// ConfigReader().Read("MINCOMMENT"));
		...
		if (!isUpdate) {
			for (Long songId = startSongId; songId <= stopSongId;) {
				...
				try {
					...
					GetSongs(onceGet.toString());
				}

				catch (Exception e1) {

					System.out.println(dateFormat.format(new Date()) + ":"
							+ (String) Thread.currentThread().getName().replace("-", "") + "," + songId
							+ " throw err:\n");
					e1.printStackTrace();
					try {
			...
			QueryBySongIdAndUpdate();
		}
	}

	public void QueryBySongIdAndUpdate() {
		while (true) {
			List<Long> list = musicDataHandler.selectSongId();
			int count = list.size();
			if (count > 0) {
				int sumCount = count / get_info_once;
				int yu = count % get_info_once;
				for (int i = 0; i < sumCount + 1; i++) {
					List<Long> listTemp = new ArrayList<Long>();
					if (sumCount != i) {
						listTemp = list.subList((i * get_info_once), (i + 1) * get_info_once);
					} else {
						if (yu > 0) {
							listTemp = list.subList((i * get_info_once), count);
						} else {
							continue;
						}
					}
					String GetSongsStr = StringUtils.join(listTemp, ",");
					 this.GetSongs(GetSongsStr);
				}
			}

			try {
				new MusicDao().fileAllOutputStream(
						new Date().toString() + " update " + count + " songs and sleep" + " 1 minute\n");
			...
			// txtlog.log(DateTime.Now + " update " + count + " songs and sleep
			// " + ConfigReader.GetAllSongIdInterval + " minutes",2);

		}
	}

	private void GetSongs(String getSongsStr) {
		try {
			// FIXME 自动生成的方法存根
			String toGet = info_url.replaceAll("#songIds#", getSongsStr.toString());

			WebResource webResource = client.resource(toGet);
			webResource.setProperty("accept", "application/json");
			ClientResponse response = webResource.header("origin", "http://music.163.com").post(ClientResponse.class);
			...
			for (JsonNode songInfo : songs) {
				...
				// new MusicDao().fileErrorOutputStream(existSongId.toString() +
				// " 有数据！\n");
				...
				try {
					...
					if (commentCountDbl == null || name == null || existSongId == null || artist == null) {
						...
						new MusicDao().fileAllOutputStream(dateFormat.format(new Date()) + ":"
								+ (String) Thread.currentThread().getName().replace("-", "") + " 有空值不插入:" + existSongId
								+ "," + name + "," + artist + "," + commentCountDbl + "\n");
					}
					Integer commentCount = commentCountDbl.intValue();
					if (commentCount >= min_Comment && !isUpdate) {
						musicDataHandler.putCount(existSongId, commentCount, name, artist);

					}
					if (isUpdate) {
						musicDataHandler.updateCount(existSongId, commentCount);
